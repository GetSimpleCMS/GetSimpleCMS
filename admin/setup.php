...
if(isset($_POST['submitted'])) {
	...
	// If the password was set and it is not blank, if it is blank it will be autogenerated
	if ($_POST['password'] && trim($_POST['password']) !== "") {

		$password = $_POST['password'];

		// Password should be 6 chars or longer.
		if (strlen($password) >= 6) {
			$PASSWD = passhash($password);
		} else {
			$err .= i18n_r('PASSWORD_ERROR') .'<br />';
		}
	}
	...
	if ($err == '')	{
		...
		if (!isset($PASSWD)) {
			...
			gs_setcookie("PASSWORD_AUTOGENERATED", "1");
		} else {
			gs_setcookie("PASSWORD_AUTOGENERATED", "0");
		}
		...
		$message .= '<br>'. i18n_r('EMAIL_PASSWORD') .': <strong>'. $password .'</strong>';
	...
	<div id="maincontent">
		<?php
			...
			if ($err != '') {
				...
				echo '<div class="updated">'.i18n_r('NOTE_USERNAME').' <b>'. stripslashes($_POST['user']) .'</b> '.i18n_r('NOTE_PASSWORD').' <b>'. $password .'</b> &nbsp&raquo;&nbsp; <a href="support.php?updated=2">'.i18n_r('EMAIL_LOGIN').'</a></div>';
				$_POST = null;
			}
	...
	if (!$success) { ?>
		<div class="main" >
			...
			<form action="<?php myself(); ?>" class="entersubmit" method="post" accept-charset="utf-8" >
				...
				<p><label for="password" ><?php i18n('LABEL_PASSWORD'); ?> (<?php i18n('LABEL_PASSWORD_OPTIONAL'); ?>):</label><input class="text" name="password" id="password" type="password" value="<?php if(isset($_POST['password'])) { echo $_POST['password']; } ?>" /></p>
